...
class VersionTest(unittest.TestCase):
    ...
    def test_parse(self):
        self.assertTupleEqual((0, 0, 0), Version.parse(b'0.0.0'))
        self.assertTupleEqual((0, 0, 0), Version.parse(b'000.000.000'))
        self.assertTupleEqual((1, 2, 3), Version.parse(b'001.002.003'))
        with self.assertRaises(ValueError):
            Version.parse(b'0.0.c')
        with self.assertRaises(ValueError):
            Version.parse(b'1.2')
        with self.assertRaises(ValueError):
            Version.parse(b'1.')
    ...
    def test_is_compatible_with(self):
        v0_1_0 = Version.parse(b'0.1.0')
        v0_2_0 = Version.parse(b'0.2.0')
        v1_0_0 = Version.parse(b'1.0.0')
    ...
    def test_newer(self):
        versions = [
            Version.parse(b'0.1.0'),
            Version.parse(b'0.1.1'),
            Version.parse(b'0.2.0'),
            Version.parse(b'0.10.0'),
            Version.parse(b'1.0.0'),
        ]
